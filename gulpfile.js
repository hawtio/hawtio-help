...
var config = {
  ...
  templateModule: 'hawtio-help-templates',
  dist: argv.out || './dist/',
  testDoc: 'plugins/help/doc/testing/',
  js: 'hawtio-help.js',
  css: 'hawtio-help.css',
...
gulp.task('clean-defs', function() {
  return del([config.dist + '*.d.ts', config.testDoc]);
});
...
gulp.task('tsc', ['clean-defs'], function() {
  ...
  return eventStream.merge(
    tsResult.js
      ...
      .pipe(gulp.dest('.')),
    tsResult.dts
      .pipe(plugins.rename('hawtio-help.d.ts'))
...
gulp.task('connect', ['watch'], function() {
  ...
  hawtio.use('/', function(req, res, next) {
          ...
          if (s.startsWith(path, '/plugins/') && (s.endsWith(path, 'html') || (s.endsWith(path, 'md')))) {
